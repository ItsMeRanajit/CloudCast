...
const WeatherDetails = ({ weather, setQuery, setUnits }) => {
  ...
  const { main, humidity, speed, pressure, visibility, feels_like, icon } = weather;
  ...
  const handleLocation = () => {
    ...
    document.getElementById("input").value = setCity("");
  };
  return (
    <div className="flex flex-col items-center w-[400px]h-[380px]">
      ...
      <div className=" text-center relative text-4xl text-white after:content-[''] after:block after:w-80 after:h-[2px] after:bg-white after:m-2">{main}</div>
      ...
      <div className="relative flex flex-col items-center justify-center gap-2 w-72">
        <input
          ...
          className="p-2 text-center text-white capitalize bg-transparent border-b-2 border-white w-52 focus:outline-none"
          value={city}
        ...
        <IoSearchCircleOutline className="absolute right-0 transition-all cursor-pointer size-8 invert hover:scale-110 top-2" onClick={handleSearchClick} onKeyDown={handleSearchClick} />
        <div className="bars">
